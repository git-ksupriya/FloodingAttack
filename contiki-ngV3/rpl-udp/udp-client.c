/* udp-client.c (contiki-ng3 client: RTT + summary) */
#include "contiki.h"
...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  while(1) {
    PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&periodic_timer) || etimer_expired(&sim_timer));
    ...
    if(etimer_expired(&sim_timer)) {
      ...
      LOG_INFO("---- CLIENT SUMMARY (10 min) ----\n");
